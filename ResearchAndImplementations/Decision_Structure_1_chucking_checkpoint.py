# chunk_driver_resume.py
# Run: python chunk_driver_resume.py
...
CONFIG = {
    ...
    "output_dir": f"Phrase_out",                     # None = next to input
...
# put near the top of Decision_Structure_1_chucking_checkpoint.py
def _clamp(a, lo, hi):
    return max(lo, min(a, hi))

def repair_field_result(fr: dict) -> dict:
    """
    Make LLM output safe:
      - ensure 0 <= start < end <= len(input)
      - if text mismatch, try to realign spans by searching the text
      - if still bad, drop the bad span
    """
    if not isinstance(fr, dict) or "input" not in fr or "sentences" not in fr:
        return {"input": fr.get("input", ""), "sentences": []}
    src = fr.get("input") or ""
    n = len(src)
    fixed_sents = []
    for s in fr.get("sentences", []):
        st, en, txt = int(s.get("start", 0)), int(s.get("end", 0)), s.get("text", "")
        st, en = _clamp(st, 0, n), _clamp(en, 0, n)
        if st >= en or not txt or src[st:en] != txt:
            # try to find the sentence text in src
            if txt:
                pos = src.find(txt)
                if pos != -1:
                    st, en = pos, pos + len(txt)
                else:
                    # drop this sentence if we cannot place it
                    continue
            else:
                # drop empty sentence
                continue
        ...
        for c in s.get("clauses", []):
            cst, cen, ctxt = int(c.get("start", st)), int(c.get("end", en)), c.get("text", "")
            cst, cen = _clamp(cst, st, en), _clamp(cen, st, en)
            ...
            for p in c.get("phrases", []):
                pst, pen, ptxt = int(p.get("start", cst)), int(p.get("end", cst)), p.get("text", "")
                pst, pen = _clamp(pst, cst, cen), _clamp(pen, cst, cen)
                # if text mismatch, try to locate the phrase text inside the clause
                if not ptxt:
                    continue
                if src[pst:pen] != ptxt:
                    ...
                    if pos != -1:
                        ...
                        continue
                # enforce non-overlap in surface order
                if pst < last_end:
                    continue
                fixed_phr.append({"type": p.get("type", "OTHER"),
                                  "text": src[pst:pen], "start": pst, "end": pen})
                last_end = pen
            fixed_clauses.append({"text": src[cst:cen], "start": cst, "end": cen, "phrases": fixed_phr})
        fixed_sents.append({"text": src[st:en], "start": st, "end": en, "clauses": fixed_clauses})
...
def load_splitter(cfg: Dict[str, Any]):
    ...
    if backend == "spacy":
        ...
        # inside load_splitter, backend == "llm"
        fn = getattr(mod, cfg["llm_fn"])
        def splitter(text: str) -> Dict[str, Any]:
            try:
                raw = fn(text)   # may raise ValueError from extract_json
            except Exception as e:
                # return a structured placeholder so the driver can continue
                return {"input": text, "sentences": [], "error": f"{type(e).__name__}: {e}"}
