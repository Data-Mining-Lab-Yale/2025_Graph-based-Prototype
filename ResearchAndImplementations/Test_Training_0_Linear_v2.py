"""
Baseline Step 1: TF-IDF + Logistic Regression on TEXT ONLY.

Inputs:
  EPPC_output_json/subsentence_goal_oriented_label.json
  EPPC_output_json/sentence_goal_oriented_label.json

Outputs per dataset under:
  EPPC_output_json/baseline_step1_text_outputs/<dataset_name>/
    - label_map.json
    - metrics.json
    - predictions.json
...
OUT_ROOT = BASE_DIR / "baseline_step1_text_outputs"
...
def vectorize(train_texts, val_texts):
    ...
    char_vec = TfidfVectorizer(analyzer="char", ngram_range=(3,5), min_df=2, max_features=200_000)
...
def save_predictions(out_dir, ids_va, texts_va, y_true_labs, y_pred_labs, y_prob, label_order):
    ...
    for i, _id in enumerate(ids_va):
        ...
        rows.append(rec)
    save_json(out_dir / "predictions.json", rows)
...
for in_path in INPUT_FILES:
    ...
    if len(texts) < 10:
        print(f"[{dataset}] Insufficient data: {len(texts)} usable, {skipped} skipped")
        continue

    label2id, id2label = make_label_maps(labels)
    y_idx = np.array([label2id[l] for l in labels], dtype=int)

    X_tr_txt, X_va_txt, y_tr, y_va, ids_tr, ids_va, texts_tr, texts_va = train_test_split(
        texts, y_idx, ids, texts, test_size=0.2, random_state=42, stratify=y_idx
    )

    X_tr_vec, X_va_vec = vectorize(X_tr_txt, X_va_txt)
    ...
    print(f"[{dataset}] usable={len(texts)} skipped={skipped} acc={acc:.3f} macro_f1={macro_f1:.3f}")
