"""
Semantic Baseline Step 2: LSA + Prototype Similarities on TEXT ONLY.

Defaults:
  EPPC_output_json/subsentence_goal_oriented_label_filtered.json
  EPPC_output_json/sentence_goal_oriented_label_filtered.json

Run:
  python Test_Training_1_Linear_text_only.py \
    --subsentence EPPC_output_json/subsentence_goal_oriented_label_filtered.json \
    --sentence EPPC_output_json/sentence_goal_oriented_label_filtered.json \
    --outdir EPPC_output_json/baseline_step2_lsa_text_outputs \
    --svd_dim 300 --test_size 0.2 --seed 42
"""
...
def load_data(path: Path):
    ...
    for k, v in data.items():
        text = (v.get("text") or "").strip()  # TEXT ONLY
        labs = v.get("labels", [])
...
def build_lsa(train_texts, val_texts, n_components=300, max_word_feats=80000):
    tfidf = TfidfVectorizer(analyzer="word", ngram_range=(1, 2), min_df=2, max_features=max_word_feats)
    X_tr_tf = tfidf.fit_transform(train_texts)
    X_va_tf = tfidf.transform(val_texts)

    n_comp = max(2, min(n_components, X_tr_tf.shape[1] - 1))
...
def add_prototype_similarities(X_tr, y_tr, X_va, n_classes):
    ...
    for c in range(n_classes):
        mask = (y_tr == c)
        centroids[c] = X_tr[mask].mean(axis=0) if np.any(mask) else 0.0
...
def train_eval(X_tr, y_tr, X_va, y_va, n_classes):
    clf = LogisticRegression(max_iter=1000, class_weight="balanced", solver="lbfgs", multi_class="auto")
    clf.fit(X_tr, y_tr)
...
def process_file(in_path: Path, out_root: Path, test_size: float, seed: int, svd_dim: int):
    ...
    for i, lab in inv.items():
        ...
        report_named[lab] = d if d else {"precision": 0.0, "recall": 0.0, "f1-score": 0.0, "support": 0}
    ...
    print(f"[{dataset}] usable={len(texts)} skipped={skipped} acc={acc:.3f} macro_f1={macro_f1:.3f} split={split_note}")
    print(f"Saved to: {out_dir}")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--subsentence", type=str, default="EPPC_output_json/subsentence_goal_oriented_label_filtered.json")
    ap.add_argument("--sentence",   type=str, default="EPPC_output_json/sentence_goal_oriented_label_filtered.json")
    ap.add_argument("--outdir",     type=str, default="EPPC_output_json/baseline_step2_lsa_text_outputs")
    ...
    if subsentence_path.exists():
        process_file(subsentence_path, out_root, args.test_size, args.seed, args.svd_dim)
    else:
        print(f"Missing file: {subsentence_path}")

    if sentence_path.exists():
        process_file(sentence_path, out_root, args.test_size, args.seed, args.svd_dim)
    else:
