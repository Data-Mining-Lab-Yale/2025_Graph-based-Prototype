...
def evaluate(model, dataloader, tokenizer, label_encoder, text_list):
    ...
    # Load reverse mapping from text to ID
    with open(INPUT_JSON, "r", encoding="utf-8") as f:
        original_data = json.load(f)

    text_to_id = {
        entry["text"]: uid
        for uid, entry in original_data.items()
        if "text" in entry
    }
    ...
    with torch.no_grad():
        for texts, labels in dataloader:
            ...
            for j in range(len(texts)):
                pred = label_encoder.inverse_transform([preds[j].cpu().item()])[0]
                gold = label_encoder.inverse_transform([labels[j].cpu().item()])[0]
                uid = text_to_id.get(texts[j], None)
                ...
                logs.append({
                    ...
                    "pred_label": pred,
                    "correct": gold == pred,
                    "sentence_id" if SOURCE_TAG == "sentence" else "subsentence_id": uid
                })

    # Save full structured error log
    with open(ERROR_LOG_PATH, "w", encoding="utf-8") as f:
        json.dump({
            "model": MODEL_NAME,
            "source": SOURCE_TAG,
