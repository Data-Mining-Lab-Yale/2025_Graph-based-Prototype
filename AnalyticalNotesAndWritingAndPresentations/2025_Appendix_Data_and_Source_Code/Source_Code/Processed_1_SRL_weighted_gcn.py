...
# ====== CONFIG ======
INPUT_FILE = "processed_srl_weighted_features.pt"
OUTPUT_DIR = "results_srl_weighted"
BATCH_SIZE = 32
NUM_EPOCHS = 200
LEARNING_RATE = 1e-3
...
train_idx, test_idx = train_test_split(indices, test_size=0.2, stratify=strat_labels, random_state=SEED)
...
error_list = []
for pred, true, sample in zip(all_pred, all_true, [dataset[i] for i in test_idx]):
    if pred != true:
        error_list.append({
            "id": sample.id,
            "text": sample.x.shape,  # Placeholder
...
plt.ylabel("Score")
plt.legend()
plt.title("SRL-GCN Weighted Training Curve")
plt.grid(True)
plt.tight_layout()
