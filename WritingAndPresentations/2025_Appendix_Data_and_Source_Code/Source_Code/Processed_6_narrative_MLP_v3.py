...
OUTPUT_DIR = "results/results_narrative_ego_mlp_3"
...
def train():
    ...
    print("\n🔍 Evaluating MSG-MLP on Held-Out Test Set (val_set)...")
    ...
    # # from sklearn.metrics import classification_report
    # report = classification_report(val_Y, preds, target_names=[id2label[i] for i in sorted(id2label)], digits=4)
    ...
    report = classification_report(val_Y, preds, labels=labels,
                                target_names=[id2label[i] for i in labels], digits=4)
    print("\n=== Classification Report (Test Set) ===")
    print(report)
    ...
    # === Save Errors (Fixed version using val_indices) ===
    errors = []
    for local_i, (pred, true) in enumerate(zip(preds, val_Y.tolist())):
        if pred != true:
            global_i = val_indices[local_i]
            meta = meta_all[global_i]
            errors.append({
