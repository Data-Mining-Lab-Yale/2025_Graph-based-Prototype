...
# ========= Config (add these) =========
LABEL_LEVEL = "code"  # options: "subcode" | "code"
SUFFIX_IN_OUTDIR = True  # if True, will write into OUTDIR + f"_{LABEL_LEVEL}"
...
OUTDIR          = "Data_for_Evidences/ambiguity_stats_code"
...
def choose_label_by_level(ann: dict, level: str):
    """
    Returns the matched label at the requested level exactly,
    or None if that level is not present on this annotation.
    """
    raw = ann.get("code", [])
    ...
    for tok in raw:
        ...
        if info.get("level") == level:
            return info.get("matched_codebook_label")
    return None
