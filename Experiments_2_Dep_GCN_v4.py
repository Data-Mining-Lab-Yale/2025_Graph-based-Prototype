...
# === Updated Early Stopping with min_delta ===
best_f1 = 0.0
epochs_no_improve = 0
early_stop_patience = 30  # Stop if no improvement for 30 epochs
min_delta = 1e-3  # Minimum F1 improvement to reset counter
    if f1 > best_f1 + min_delta:
        ...
        if epochs_no_improve >= early_stop_patience:
            print(f"🛑 Early stopping at epoch {epoch+1}. No improvement over {early_stop_patience} epochs.")
            break
        if epochs_no_improve >= early_stop_patience:
...
# Wrap the training log with metadata for consistency
# wrapped_log = {
#     "model": "DEP-GCN",
#     "source": "subsentence",
#     "label_type": "subcode",
#     "seed": SEED,
#     "input_file": graph_label_path,
...
wrapped_log = {
    ...
    "label_type": "subcode",
    "seed": SEED,
    "input_file": GRAPH_DIR,  # the folder used for input graphs
    "class_names": list(label_encoder.classes_),
    "metrics": train_log
}
with open(os.path.join(OUTPUT_DIR, "train_log.json"), "w") as f:
    json.dump(wrapped_log, f, indent=2)
